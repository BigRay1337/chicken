function pageScript() {
  ...
  // Detect a Play button click and then wait for the website to change.
  let waitingForPlayPageChange = false;
  let playPageChangeTimer = null;
  let playPageChangeObserver = null;
  let playClickUrl = window.location.href;
  let playClickBodySignature = "";

  const getPageSignature = () => {
    const body = document.body;
    if (!body) return "";
    return `${window.location.href}|${body.innerText}|${body.children.length}|${body.innerHTML.length}`;
  };
  ...
  const isPlayButton = (element) => {
    ...
    if (tag !== "button" && tag !== "input" && element.getAttribute("role") !== "button") {
      return false;
    }
    ...
    return /(^|\s)play(\s|$)/i.test(text) ||
      text.includes("play now") ||
      text.includes("start game");
  };

  const stopPlayPageChangeDetection = () => {
    waitingForPlayPageChange = false;

    if (playPageChangeTimer !== null) {
      originalclearTimeout(playPageChangeTimer);
      playPageChangeTimer = null;
    }

    if (playPageChangeObserver !== null) {
      playPageChangeObserver.disconnect();
      playPageChangeObserver = null;
    }
  };

  const disableDateNowAfterPlayPageChange = () => {
    if (!waitingForPlayPageChange || !speedConfig.cbDateNowChecked) return;

    stopPlayPageChangeDetection();
    speedConfig = { ...speedConfig, cbDateNowChecked: false };

    // Tell the content script so its runtime copy of speedConfig is also updated.
    window.postMessage({
      command: "dateNowPageChanged",
      config: speedConfig,
    });

    // Apply the change immediately in the page context and keep the existing
    // reload behavior used whenever Date.now is disabled.
    window.postMessage({
  ...
  const startPlayPageChangeDetection = () => {
    if (!speedConfig.cbDateNowChecked) return;

    stopPlayPageChangeDetection();
    waitingForPlayPageChange = true;
    playClickUrl = window.location.href;
    playClickBodySignature = getPageSignature();

    // Give the click handler a moment to finish before observing the result.
    playPageChangeTimer = originalSetTimeout(() => {
      playPageChangeTimer = null;

      if (!waitingForPlayPageChange) return;

      playPageChangeObserver = new MutationObserver(() => {
        if (!waitingForPlayPageChange) return;

        const currentSignature = getPageSignature();
        if (currentSignature !== playClickBodySignature || window.location.href !== playClickUrl) {
          disableDateNowAfterPlayPageChange();
        }
      });

      const target = document.documentElement || document.body;
      if (target) {
        playPageChangeObserver.observe(target, {
          subtree: true,
          childList: true,
          characterData: true,
          attributes: true,
      ...
      // Also catch SPA URL changes that do not cause a DOM mutation.
      const checkUrl = () => {
        if (!waitingForPlayPageChange) return;
        if (window.location.href !== playClickUrl) {
          disableDateNowAfterPlayPageChange();
          return;
        }
        playPageChangeTimer = originalSetTimeout(checkUrl, 100);
      };
      playPageChangeTimer = originalSetTimeout(checkUrl, 100);
    }, 50);
  };
  ...
  document.addEventListener("click", (event) => {
    ...
    const playButton = element.closest("button, input, [role='button']");
    if (playButton && isPlayButton(playButton)) {
      startPlayPageChangeDetection();
    }
  }, true);
